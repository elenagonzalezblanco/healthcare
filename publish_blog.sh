...
LOG="$HOME/Library/Logs/nakaia-blog.log"
{
  ...
  if ls news/*"$TODAY"*.html >/dev/null 2>&1; then
    ...
    /usr/bin/osascript -e 'display notification "No encuentro el brief de hoy en la carpeta local (Escritorio → Nakaia-briefs). El blog no se ha actualizado con noticias nuevas." with title "ELEVALOS · Blog" sound name "Ping"' || true
    touch "$FLAG"
